    termController.$inject = ['$scope', '$rootScope', '$state', 'saveService'];

    angular
        .module('app.myTerm')
        .controller('termController', termController);

    function termController($scope, $rootScope, $state, saveService) {

        $scope.selectedTerm;
        var selected = false;

        saveService.loadSavedSchedules();        
        $scope.schedules = saveService.getSavedSchedules();
